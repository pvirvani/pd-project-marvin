...
|
| Modified by Pardeep Kumar, LIG lab/ Marven Team, France, 2023.
| Implementation of LOCM3
'''
...
def domain_gen(sequences,project):
    domain_name = project+'_domain'  # TO-DO extract the name from name given during project creation 
    ...
    input_sequence = input_sequence.strip()[:-1]
    sequences = read_input(input_sequence)
    print('input_sequence variable:', sequences)
    ...
    f.close()
    with open("output/"+ domain_name + "/" +  domain_name + ".pddl",'r') as domain_file:
        pddl_content = domain_file.read()
    return pddl_content